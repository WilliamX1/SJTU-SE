...
long long gen_f_pc()
{
    return ((((((((if_id_curr->icode) == (I_JMP)) & ((if_id_next->ifun) == 1
                  )) & ((id_ex_curr->icode) == (I_ALU))) & (
              (id_ex_curr->ifun) == 2)) & ((id_ex_curr->srca) == 
            (id_ex_curr->srcb))) & ((id_ex_curr->vala) > 0)) ? 
      (if_id_curr->valp) : ((((ex_mem_curr->icode) == (I_JMP)) & !
          (ex_mem_curr->takebranch)) & !((((ex_mem_curr->ifun) == 1) & (
              (mem_wb_curr->icode) == (I_ALU))) & ((mem_wb_curr->ifun) == 2)
          )) ? (ex_mem_curr->vala) : (((if_id_curr->icode) == (I_RET)) & (
          (id_ex_curr->icode) == (I_PUSHQ))) ? (id_ex_curr->vala) : ((
          (mem_wb_curr->icode) == (I_RET)) & ((ex_mem_curr->icode) == 
          (I_NOP))) ? (mem_wb_curr->valm) : (pc_curr->pc));
}
