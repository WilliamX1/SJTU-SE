...
#	 1, put IADDQ into 'instr_valid';
# 	 2, because iaddq needs one register, so put it into 'need_regids';
# 	 3, because iaddq needs one immediate, so put it into 'need_valC';
#	 4, as iaddq needs regB, so d_srcB for iaddq should be 'D_rB';
#	 5, because iaddq will write the caculated value back into regB, so d_dstE for iaddq should be 'D_rB';
# 	 6, referred to the details of add, so aluA for iaddq should be 'E_valC', while aluB for iaddq should be 'E_valB';
# 	 7, for the future, there may be some conditions for iaddq to execute, so I add it into 'alufun' just like IOPQ;
# 	 8, when we use iaddq, it may change cc, so I add it into 'set_cc';
# 	 and that's all in this file, other changes are about ncopy.ys and will be described in ncopy.ys;
#    9, change the source of e_valA, so it can improve pipeline to "load forwarding";
#    10, For the instructions "andq", "jle", they can predict branch with higher accuracy;
# 	 11, optimize the instruction "pushq" that appeared in Jump table, now it need only 1 bubble between pushq and ret;
