...
SyntaxTree::node::node(QString s){
    ...
    else if (val == "(") type = OpLeftBracket;
    else if (val == ")") type = OpRightBracket;
    /*大于优先级很低*/
...
void SyntaxTree::normalLize(QString& str){
    /*防止首字母就是运算符导致数组越界*/
    str = " " + str.simplified() + " ";
    for (int i = 0; str[i] != '\0'; i++){
        /*特殊考虑乘方*/
        if (str[i] == '*' && (i + 1) <= str.length() && str[i + 1] == '*') {
            str = str.mid(0, i) + " " + str[i] + str[i + 1] + " " + str.mid(i + 2, -1);
            i += 3;
        }
        /*凡是运算符前后均加空格*/
                 ...
                 || str[i] == '>' || str[i] == '<') {
            str = str.mid(0, i) + " " + str[i] + " " + str.mid(i + 1, -1);
            i += 2;
        } else continue;
    }
    str = str.simplified();
...
bool SyntaxTree::convertToPostfixExp(QString& errorName){
    std::stack<node*> _saved;
    QString tmp = _contents.simplified();
    while(tmp != "") {
        /*识别每一个并存入节点集*/
        node* newnode = new node(tmp.section(' ', 0, 0).simplified());

        switch (newnode->type) {
        /*如果遇到操作数，则直接入后缀栈*/
        case Imm: case Var: _res.push(newnode); break;
